# -*- coding: utf-8 -*-
"""
Created on Thu Apr 25 13:37:45 2024
...
def system_optimizer(A,B):
    #*************************************** CONSTANTS SECTION ********************************************************************************************************************************************

    Arrays = A # super simple parameter for PV array output
    Bateries = B
    ...
    cells = 72 * Arrays             # total cells in system (simplification)

    Pmax = 285 * A                  # total wattage (sticker) of system
    ...
    Average_Bsoc = 100*np.mean(BSOC)
    Lowest_BSOC  = np.min(BSOC*100)
    Energy_Consumed = fraction_used_solar*100
    
    Cost = 2.5*285*A + 1250*B + 1350
    
    return Availability,Average_Bsoc,Lowest_BSOC,Energy_Consumed,Cost
...
inputs = np.array([['Arrays','Batteries']])
data = np.array([['Availability','Average_Bsoc','Lowest_BSOC','Energy_Consumed','Cost']]) 

for a in A:
    for b in B:
        data = np.vstack([data,system_optimizer(a,b)])
        inputs = np.vstack([inputs,[a,b]])
        index = np.vstack([index,[i]])
        i += 1
        
full_data = np.hstack([index,inputs,data])
        
Solar_Frame = pd.DataFrame(data = full_data[1:,1:],index = full_data[1:,0],columns = full_data[0,1:])

print(Solar_Frame)

Solar_Frame['Availability'] = pd.to_numeric(Solar_Frame['Availability'])
Solar_Frame['Average_Bsoc'] = pd.to_numeric(Solar_Frame['Average_Bsoc'])
Solar_Frame['Lowest_BSOC'] = pd.to_numeric(Solar_Frame['Lowest_BSOC'])
Solar_Frame['Energy_Consumed'] = pd.to_numeric(Solar_Frame['Energy_Consumed'])
Solar_Frame['Cost'] = pd.to_numeric(Solar_Frame['Cost'])

good_ones = Solar_Frame[Solar_Frame['Availability'] > 99.6]


print("Optimal Components:")
print(good_ones.iloc[:1].to_string())

Solar_Frame.plot(x='Availability', y='Cost', style='o')
